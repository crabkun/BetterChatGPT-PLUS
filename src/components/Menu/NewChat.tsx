...
const NewChat = ({ folder }: { folder?: string }) => {
  ...
  return (
    <a
      className={`flex flex-1 items-center rounded-md hover:bg-gray-500/10 transition-all duration-200 text-white text-sm flex-shrink-0 cursor-pointer opacity-100 ${folder ? 'justify-start' : 'py-2 px-2 gap-3 mb-2 border border-white/20'
        }`}
      onClick={() => {
        addChat(folder);
      }}
